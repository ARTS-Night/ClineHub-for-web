...
import { A as r, F as i, b as a, s as o, z as s } from "./chunk-I66GZJ75-DaJZKFkN.js";
import { a as c } from "./chunk-NSK5VX7P-pGKppjHe.js";
import { n as l, t as u } from "./chunk-4I5QYGJK-_sTbXS-x.js";
...
var g = (() => {
	...
	lheading: oe,
	table: fe,
	paragraph: h(b).replace("hr", v).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", fe).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", S).getRegex()
}, me = {
	...w,
	...
	fences: m,
	lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
	paragraph: h(b).replace("hr", v).replace("heading", " *#{1,6} *[^\n]").replace("lheading", ae).replace("|table", "").replace("blockquote", " {0,3}>").replace("|fences", "").replace("|list", "").replace("|html", "").replace("|tag", "").getRegex()
}, he = /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/, ge = /^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/, _e = /^( {2,}|\\)\n(?!\s*$)/, ve = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/, T = /[\p{P}\p{S}]/u, E = /[\s\p{P}\p{S}]/u, ye = /[^\s\p{P}\p{S}]/u, be = h(/^((?![*_])punctSpace)/, "u").replace(/punctSpace/g, E).getRegex(), xe = /(?!~)[\p{P}\p{S}]/u, Se = /(?!~)[\s\p{P}\p{S}]/u, Ce = /(?:[^\s\p{P}\p{S}]|~)/u, we = h(/link|precode-code|html/, "g").replace("link", /\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/).replace("precode-", g ? "(?<!`)()" : "(^^|[^`])").replace("code", /(?<b>`+)[^`]+\k<b>(?!`)/).replace("html", /<(?! )[^<>]*?>/).getRegex(), D = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/, Te = h(D, "u").replace(/punct/g, T).getRegex(), Ee = h(D, "u").replace(/punct/g, xe).getRegex(), O = "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)", De = h(O, "gu").replace(/notPunctSpace/g, ye).replace(/punctSpace/g, E).replace(/punct/g, T).getRegex(), Oe = h(O, "gu").replace(/notPunctSpace/g, Ce).replace(/punctSpace/g, Se).replace(/punct/g, xe).getRegex(), ke = h("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)", "gu").replace(/notPunctSpace/g, ye).replace(/punctSpace/g, E).replace(/punct/g, T).getRegex(), Ae = h(/\\(punct)/, "gu").replace(/punct/g, T).getRegex(), je = h(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(), Me = h(C).replace("(?:-->|$)", "-->").getRegex(), Ne = h("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", Me).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(), k = /(?:\[(?:\\[\s\S]|[^\[\]\\])*\]|\\[\s\S]|`+[^`]*?`+(?!`)|[^\[\]\\`])*?/, Pe = h(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label", k).replace("href", /<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(), A = h(/^!?\[(label)\]\[(ref)\]/).replace("label", k).replace("ref", x).getRegex(), j = h(/^!?\[(ref)\](?:\[\])?/).replace("ref", x).getRegex(), Fe = h("reflink|nolink(?!\\()", "g").replace("reflink", A).replace("nolink", j).getRegex(), Ie = /[hH][tT][tT][pP][sS]?|[fF][tT][pP]/, M = {
	_backpedal: m,
	anyPunctuation: Ae,
	autolink: je,
	blockSkip: we,
	...
	...M,
	link: h(/^!?\[(label)\]\((.*?)\)/).replace("label", k).getRegex(),
	reflink: h(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", k).getRegex()
}, N = {
	...M,
...
function I(e, t) {
	if (t) {
		if (_.escapeTest.test(e)) return e.replace(_.escapeReplace, Be);
	} else if (_.escapeTestNoEncode.test(e)) return e.replace(_.escapeReplaceNoEncode, Be);
	return e;
}
function Ve(e) {
	try {
...
function He(e, t) {
	let n = e.replace(_.findPipe, (e, t, n) => {
		let r = !1, i = t;
		for (; --i >= 0 && n[i] === "\\";) r = !r;
		return r ? "|" : " |";
	}).split(_.splitPipe), r = 0;
...
${c}` : c;
	...
	table(e) {
		...
		let n = He(t[1]), r = t[2].replace(this.rules.other.tableAlignChars, "").split("|"), i = t[3]?.trim() ? t[3].replace(this.rules.other.tableRowBlankLine, "").split("\n") : [], a = {
			type: "table",
			raw: t[0],
			header: [],
			align: [],
		...
		if (n.length === r.length) {
			...
			for (let e of i) a.rows.push(He(e, a.header.length).map((e, t) => ({
				text: e,
				tokens: this.lexer.inline(e),
				header: !1,
				align: a.align[t]
	...
	inlineQueue;
	constructor(e) {
		this.tokens = [], this.tokens.links = Object.create(null), this.options = e || f, this.options.tokenizer = this.options.tokenizer || new R(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = {
			inLink: !1,
			inRawBlock: !1,
			top: !0
		};
	...
	code({ text: e, lang: t, escaped: n }) {
		let r = (t || "").match(_.notSpaceStart)?.[0], i = e.replace(_.endingNewline, "") + "\n";
...
`;
	...
	textRenderer;
	constructor(e) {
		this.options = e || f, this.options.renderer = this.options.renderer || new B(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.renderer.parser = this, this.textRenderer = new V();
	}
	static parse(t, n) {
...
G.options = G.setOptions = function(e) {
	return W.setOptions(e), G.defaults = W.defaults, p(G.defaults), G;
}, G.getDefaults = d, G.defaults = f, G.use = function(...e) {
	return W.use(...e), G.defaults = W.defaults, p(G.defaults), G;
...
function qe(e, { markdownAutoWrap: t }) {
	return Ke(e.replace(/<br\/>/g, "\n").replace(/\n{2,}/g, "\n"));
}
e(qe, "preprocessMarkdown");
function Je(e) {
...
e(Je, "nonMarkdownToLines");
function Ye(t, n = {}) {
	let r = qe(t, n), i = G.lexer(r), a = [[]], o = 0;
	...
	return e(s, "processNode"), i.forEach((e) => {
...
e(Ye, "markdownToLines");
function Xe(e) {
	return e ? `<p>${e.replace(/\\n|\n/g, "<br />")}</p>` : "";
}
e(Xe, "nonMarkdownToHTML");
function Ze(n, { markdownAutoWrap: r } = {}) {
	let i = G.lexer(n);
	function a(e) {
		return e.type === "text" ? r === !1 ? e.text.replace(/\n */g, "<br/>").replace(/ /g, "&nbsp;") : e.text.replace(/\n */g, "<br/>") : e.type === "strong" ? `<strong>${e.tokens?.map(a).join("")}</strong>` : e.type === "em" ? `<em>${e.tokens?.map(a).join("")}</em>` : e.type === "paragraph" ? `<p>${e.tokens?.map(a).join("")}</p>` : e.type === "space" ? "" : e.type === "html" ? `${e.text}` : e.type === "escape" ? e.text : (t.warn(`Unsupported markdown: ${e.type}`), e.raw);
	}
	return e(a, "output"), i.map(a).join("");
}
e(Ze, "markdownToHTML");
function Qe(e) {
	return Intl.Segmenter ? [...new Intl.Segmenter().segment(e)].map((e) => e.segment) : [...e];
}
e(Qe, "splitTextToChars");
function $e(e, t) {
	return K(e, [], Qe(t.content), t.type);
}
e($e, "splitWordToFitWidth");
function K(e, t, n, r) {
	...
	return e([{
		content: o.join(""),
		type: r
	}]) ? K(e, o, a, r) : (t.length === 0 && i && (t.push(i), n.shift()), [{
		content: t.join(""),
		type: r
	}, {
		content: n.join(""),
...
e(q, "splitLineToFitWidthRecursion");
function J(e, t) {
	t && e.attr("style", t);
}
e(J, "applyStyle");
var tt = 16384;
async function Y(e, t, n, c, l = !1, u = a()) {
	let d = e.append("foreignObject");
	d.attr("width", `${Math.min(10 * n, tt)}px`), d.attr("height", `${Math.min(10 * n, tt)}px`);
	...
	h.html(p), J(h, t.labelStyle), h.attr("class", `${m} ${c}`), J(f, t.labelStyle), f.style("display", "table-cell"), f.style("white-space", "nowrap"), f.style("line-height", "1.5"), n !== Infinity && (f.style("max-width", n + "px"), f.style("text-align", "center")), f.attr("xmlns", "http://www.w3.org/1999/xhtml"), l && f.attr("class", "labelBkg");
...
e(rt, "computeDimensionOfText");
function it(t, n, r, i = !1, a = !1) {
	...
	let u = 0;
	for (let n of r) {
		let r = /* @__PURE__ */ e((e) => nt(s, o, e) <= t, "checkWidth"), i = r(n) ? [n] : et(n, r);
...
e(Q, "updateTextContentAndStyles");
async function $(e, t = {}) {
	let n = [];
	e.replace(/(fa[bklrs]?):fa-([\w-]+)/g, (e, r, i) => (n.push((async () => {
		let n = `${r}:${i}`;
		return await l(n) ? await u(n, void 0, { class: "label-icon" }) : `<i class='${s(e, t).replace(":", " ")}'></i>`;
	})()), e));
	let r = await Promise.all(n);
	return e.replace(/(fa[bklrs]?):fa-([\w-]+)/g, () => r.shift() ?? "");
}
e($, "replaceIconSubstring");
var at = /* @__PURE__ */ e(async (e, i = "", { style: a = "", isTitle: o = !1, classes: s = "", useHtmlLabels: l = !0, markdown: u = !0, isNode: d = !0, width: f = 200, addSvgBackground: p = !1 } = {}, m) => {
	if (t.debug("XYZ createText", i, a, o, s, l, d, "addSvgBackground: ", p), l) {
		let t = u ? Ze(i, m) : Xe(i), n = await $(c(t), m), o = i.replace(/\\\\/g, "\\");
		return await Y(e, {
			isNode: d,
			label: r(i) ? o : n,
	...
	{
		let t = c(i.replace(/<br\s*\/?>/g, "<br/>")), r = it(f, e, u ? Ye(t.replace("<br>", "<br/>"), m) : Je(t), i ? p : !1, !d);
		if (d) {
...
//#endregion
export { at as n, Ke as r, rt as t };
