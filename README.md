...
作業フォルダーは絶対パスで直接入力するほか、`参照`ボタンでサーバー側のフォルダー階層をUI上で辿って選べます（`..`で上の階層へ、フォルダー名クリックで下の階層へ）。同じ`参照`ボタンは`プロファイル`のワークスペース追加画面にもあります。既定では起動した場所以外も自由に選べます（存在するディレクトリであることのみ検証し、シンボリックリンクやジャンクションは実体パスに解決されます）。LAN上の他端末からもこのサーバーへアクセスできる構成など、選択できる範囲を1つのサブツリーに制限したい場合だけ、起動前に許可範囲を明示します。この制限はUIのフォルダー参照にも適用されます。
...
Sessionはそれぞれ作成時のモデル・ワークスペースプロファイルを記録しています。Sessionを開くと、そのプロファイルが現在の選択と異なる場合は自動で切り替わります。記録されたプロファイルが削除済みの場合は切り替えを行わず、会話上に「〇〇は削除されています。別の△△を選択してください」というメッセージを表示します。
...
- Session選択時に、記録されたモデル/ワークスペースプロファイルへ自動復元（削除済みの場合はエラー表示）
